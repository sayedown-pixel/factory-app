...
def save_session(username, password, remember=True):
    if remember:
        session_data = {
            "username": username,
            "password": password,
...
LANG = {
    "ar": {
        ...
        "info_text": "سيتم حفظ بيانات الدخول في ملف مؤقت على الجهاز لمدة 30 يوماً",
        "shipping": "شحن منتجات",
        "customer": "اسم العميل",
    ...
    "en": {
        ...
        "info_text": "Login data will be saved in a temporary file for 30 days",
        "shipping": "Shipping",
    ...
    "bn": {
        ...
        "info_title": "ℹ️ তথ্য",
        "info_text": "লগইন ডেটা ৩০ দিনের জন্য সংরক্ষিত থাকবে",
...
def calculate_pallet_count(product_name, balance, unit):
    """حساب عدد الباليتات تلقائياً بناءً على المنتج والوحدة"""
...
def show_dashboard(df_main, df_raw, df_fg, t):
    ...
    # الصف الأول: بطاقات الإنتاج العامة
    ...
    # الصف الثالث: رسمين بيانيين منفصلين - المواد الخام
    ...
    # الصف الرابع: رسم بياني - المنتج التام
...
# ============================================================================
def login_screen(t):
    saved_user, saved_pass = load_session()
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button(t["login_btn"], use_container_width=True):
                if username in USERS and USERS[username]["password"] == password:
                    ...
                    if remember:
                        save_session(username, password, True)
                    st.rerun()
                else:
...
def main():
    ...
    # ========== Records (آخر 10 أيام فقط) ==========
    elif selected_raw == "📊 Records":
        st.header(t["records"])
        
        if df_main is not None and not df_main.empty:
            # حساب تاريخ 10 أيام مضت
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v11.5<br>{t['designer']}</small></center>", unsafe_allow_html=True)
