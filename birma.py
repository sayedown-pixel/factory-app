...
# --- 2. إنشاء الاتصال (يعتمد كلياً على الأسرار Secrets) ---
    ...
    with st.form("main_form", clear_on_submit=True):
        ...
        if st.form_submit_button("إرسال التقرير"):
            if not signature:
                ...
                try:
                    # جلب الرابط من Secrets (للتأكد من استخدامه مع المفتاح)
                    ...
                    st.success("✅ تم التحديث بنجاح في Google Sheets!")
                except Exception as e:
                    st.error(f"حدث خطأ أثناء المزامنة: {e}")
            else:
                st.warning("لم يتم اختيار مهام.")

# --- 7. عرض البيانات للتأكد ---
if st.sidebar.checkbox("مراجعة السجل أونلاين"):
    try:
        url = st.secrets["connections"]["gsheets"]["spreadsheet"]
