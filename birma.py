...
# --- 1. إعدادات الصفحة ---
st.set_page_config(
    page_title="نظام صيانة بيرما الموحد",
    page_icon="🏭",
    layout="centered"
)
...
production_line = st.sidebar.selectbox("🚩 اختر خط الإنتاج", ["الخط الأول", "الخط الثاني"])
...
if engine_data is not None:
    with st.form("unified_form", clear_on_submit=True):
        ...
        for i, row in daily_tasks.iterrows():
            with st.container():
                ...
                with col1:
                    ...
                    is_done = st.checkbox("تم الفحص", key=f"chk_{i}_{production_line}")
                with col2:
                    comment = st.text_input("ملاحظات", key=f"txt_{i}_{production_line}")
        ...
        signature = st.text_input("✍️ توقيع المشغل المسؤول:")

        if st.form_submit_button(f"إرسال تقرير {production_line}", use_container_width=True):
            if not signature:
                st.error("❌ يرجى كتابة الاسم قبل الإرسال")
                ...
                st.warning("⚠️ لم يتم اختيار أي مهام.")

# عرض السجل المباشر في الأسفل
if st.sidebar.checkbox("👁 عرض السجل الأخير"):
    try:
