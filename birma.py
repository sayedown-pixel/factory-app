...
def main():
    ...
    with st.sidebar.expander("🔒 " + t["admin_title"]):
        ...
        if pw in ["admin123", "100"]:
            ...
            if not df_prod.empty:
                if lang == "ar":
                    df_prod['desc'] = df_prod.apply(lambda row: f"📦 {row['date']} | {row['product']} | {row['output_units']} {t['quantity']}", axis=1)
                else:
                    df_prod['desc'] = df_prod.apply(lambda row: f"📦 {row['date']} | {row['product']} | {row['output_units']} {t['quantity']}", axis=1)
                
                selected_idx = st.selectbox("اختر السجل للحذف", options=df_prod.index, format_func=lambda x: df_prod.loc[x, 'desc'])
                selected_row = df_prod.loc[selected_idx]
                ...
                if st.button("🗑️ " + t["delete_btn"], use_container_width=True):
                    ...
                    delete_production_by_id(selected_row['id'])
