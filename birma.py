...
st.set_page_config(page_title="BIRMA Factory Production Pro MAINTENANCE ", layout="centered")
...
if engine_data is not None:
    with st.form("operation_form"):
        ...
        # --- SIGNATURE SECTION ---
        st.subheader("✍️توقيع المشغل")
        signature = st.text_input("الاسم بالكامل:", placeholder="Type your name to sign...")

        if st.form_submit_button("ارسال التقرير"):
            if not signature:
                st.error("❌ Error: لازم التوقيع قبل الارسال!")
