...
def create_default_raw_materials():
    default_data = {
        "Material_ID": [1001, 1002, 1003, 1004, 1005, 1006],
        "Material_Name_AR": ["بريفورم 200 مل", "بريفورم 330 مل", "غطاء", "كرتون", "شرنك", "فواصل شرنك"],
        "Current_Stock": [500000, 300000, 1000000, 50000, 5000, 5000],
        "Min_Stock": [100000, 50000, 200000, 10000, 1000, 500],
...
ADHESIVE_CONSUMPTION = {
    ...
    "1.5 L Shrink": 0.35, "330 ml Carton": 0.35, "330 ml Shrink": 0.35,
}
...
def load_finished_goods():
    if os.path.exists(FINISHED_GOODS_FILE):
        try:
            ...
            # توحيد أسماء الأعمدة
            rename_map = {
                'Code': 'Code', 'Name': 'Name', 'In': 'In',
                'Out': 'Out', 'Balance': 'Balance', 'Unit': 'Unit',
                'Pallet_Count': 'Pallet_Count', 'Last_Updated': 'Last_Updated'
            }
            for old, new in rename_map.items():
                if old in df.columns:
                    df = df.rename(columns={old: new})
            
            required_cols = ["Code", "Name", "In", "Out", "Balance", "Unit", "Pallet_Count", "Last_Updated"]
            for col in required_cols:
                if col not in df.columns:
                    df[col] = 0 if col != "Name" and col != "Unit" and col != "Last_Updated" else ""
...
def add_to_finished_goods(product_name, quantity, df_fg):
    ...
    return df_fg, True, f"✅ تم إضافة {quantity:,.0f} وحدة إلى المخزن (الوارد: {old_in:,.0f} → {new_in:,.0f}, الرصيد: {old_balance:,.0f} → {new_balance:,.0f})"
...
def main():
                ...
                if target <= 0:
                    ...
                    if not raw_ok:
                        ...
                        if update_raw_materials(new_raw):
                            ...
                            # 3. إضافة المنتج إلى مخزن الإنتاج التام
                            ...
                            st.error("فشل في تحديث المواد الخام")
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v13.0<br>{t['designer']}</small></center>", unsafe_allow_html=True)
