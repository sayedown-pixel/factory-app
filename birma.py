...
USERS = {
    ...
    "pro": {"password": "200", "role": "supervisor", "name": "مشرف إنتاج", "icon": "👔"},
    "tec": {"password": "300", "role": "technician", "name": "فني صيانة", "icon": "🔧"},
    "sto": {"password": "400", "role": "storekeeper", "name": "أمين مخزن", "icon": "📦"},
...
ROLE_PERMISSIONS = {
    ...
    "pro": ["🏠 Dashboard", "📈 Production", "🔧 Maintenance", "📊 Records", "📦 Raw Materials", "🏭 Finished Goods"],
    "tec": ["🏠 Dashboard", "🔧 Maintenance", "📊 Records"],
    "sto": ["🏠 Dashboard", "📦 Raw Materials", "🏭 Finished Goods", "📊 Records"],
...
# ============================================================================
# 10. دوال الصيانة والماكينات (مع إضافة الكمبروسر)
# ============================================================================
MACHINE_MAP = {
...
def show_dashboard(df_main, df_raw, df_fg, t):
    ...
    st.markdown('<h1 class="dashboard-title">🏭 BIRMA - ' + t["dashboard_title"] + '</h1>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col1:
        ...
        <div style="background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 15px; padding: 15px; text-align: center; color: white;">
            ...
            <h1 style="font-size: 2.5rem;">{total_prod:,}</h1>
            <small>إجمالي الإنتاج منذ البداية</small>
        </div>
        """, unsafe_allow_html=True)
    
    with col2:
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #f59e0b, #d97706); border-radius: 15px; padding: 15px; text-align: center; color: white;">
            <h3>📅 {t['monthly_production']}</h3>
            <h1 style="font-size: 2.5rem;">{monthly_prod:,}</h1>
            <small>{datetime.now().strftime('%B %Y')}</small>
        </div>
        """, unsafe_allow_html=True)
    
    with col3:
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #10b981, #059669); border-radius: 15px; padding: 15px; text-align: center; color: white;">
            <h3>🏭 {t['fg_balance']}</h3>
            <h1 style="font-size: 2.5rem;">{fg_balance:,}</h1>
            <small>وحدة</small>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="text-align: center;">
            ...
            <div style="position: relative; width: 150px; height: 150px; margin: auto;">
                <svg width="150" height="150" viewBox="0 0 150 150">
                    ...
                    <circle cx="75" cy="75" r="65" fill="none" stroke="{color1}" stroke-width="12" 
                            stroke-dasharray="{2 * math.pi * 65 * line1_efficiency / 100} {2 * math.pi * 65}"
                            stroke-dashoffset="{2 * math.pi * 65 * 0.25}" transform="rotate(-90 75 75)"/>
                    <text x="75" y="80" text-anchor="middle" font-size="24" font-weight="bold" fill="{color1}">{line1_efficiency}%</text>
                    <text x="75" y="100" text-anchor="middle" font-size="12" fill="#6b7280">السرعة</text>
                </svg>
            </div>
            <small>عدد السجلات: {line1_count}</small>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div style="text-align: center;">
            ...
            <div style="position: relative; width: 150px; height: 150px; margin: auto;">
                <svg width="150" height="150" viewBox="0 0 150 150">
                    <circle cx="75" cy="75" r="65" fill="none" stroke="#e5e7eb" stroke-width="12"/>
                    <circle cx="75" cy="75" r="65" fill="none" stroke="{color2}" stroke-width="12" 
                            stroke-dasharray="{2 * math.pi * 65 * line2_efficiency / 100} {2 * math.pi * 65}"
                            stroke-dashoffset="{2 * math.pi * 65 * 0.25}" transform="rotate(-90 75 75)"/>
                    <text x="75" y="80" text-anchor="middle" font-size="24" font-weight="bold" fill="{color2}">{line2_efficiency}%</text>
                    <text x="75" y="100" text-anchor="middle" font-size="12" fill="#6b7280">السرعة</text>
                </svg>
            </div>
            <small>عدد السجلات: {line2_count}</small>
        </div>
        """, unsafe_allow_html=True)
...
def main():
    ...
    # ========== Maintenance ==========
        ...
        if m_type == t["maint_types"][0]:
            ...
            try:
                ...
                if 'Name' not in df_tasks.columns and 'Name' in df_tasks.columns:
                    pass
                elif 'Name' not in df_tasks.columns and 'Task' in df_tasks.columns:
    ...
    # ========== Records (آخر 10 أيام مع عدد البريفورم) ==========
            ...
            with tab1:
                ...
                if not prod_logs.empty and 'Date' in prod_logs.columns:
                    ...
                    if not prod_logs_last10.empty:
                        # تحديد الأعمدة للعرض مع البريفورم
                        display_cols = ['Date', 'Line', 'Supervisor', 'Product', 'Output_Units']
                        if 'Preforms_Used' in prod_logs_last10.columns:
                            display_cols.append('Preforms_Used')
                        display_cols.append('Efficiency_%')
                        
                        display_df = prod_logs_last10[display_cols].copy()
                        display_df = display_df.rename(columns={
                            'Date': 'التاريخ', 'Line': 'الخط', 'Supervisor': 'المشرف',
                            'Product': 'المنتج', 'Output_Units': 'الكمية', 
                            'Preforms_Used': 'البريفورم المستخدم', 'Efficiency_%': 'الكفاءة'
                        })
            ...
            with tab2:
                ...
                if not maint_logs.empty and 'Date' in maint_logs.columns:
                    ...
                    if not maint_logs_last10.empty:
                        st.dataframe(maint_logs_last10[['Date', 'Type', 'Machine', 'Technician', 'Task']], use_container_width=True)
