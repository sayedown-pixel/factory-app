...
# ============================================================================
# 6. نظام اللغات (مختصر للحفاظ على المساحة)
# ============================================================================
LANG = {
    "ar": {
...
def consume_materials(product, quantity, df_raw):
    """صرف المواد من المخزون (مع إمكانية تتبع للسحب)"""
...
def restore_finished_goods(product_name, quantity, df_fg):
    ...
    new_in = max(0, old_in - quantity)  # لا يمكن أن يصبح الوارد سالباً
    new_balance = old_balance - quantity
...
def main():
    ...
    # ========== Delete Records (مع إعادة المواد الخام والمنتج التام) ==========
    st.sidebar.divider()
                ...
                if 'Type' in df_display.columns and 'Date' in df_display.columns:
                    ...
                    if st.button("🗑️ " + t["delete_btn"], use_container_width=True):
                        ...
                        if selected_row['Type'] == 'Production':
                            ...
                            # حذف سجل صيانة فقط (لا يؤثر على المخزون)
                            df_updated = df_main.drop(selected_idx)
                            if conn and save_to_sheet(conn, df_updated):
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v12.1<br>{t['designer']}</small></center>", unsafe_allow_html=True)
