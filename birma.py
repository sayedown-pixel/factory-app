...
<style>
    ...
    /* تنسيق البطاقات - خلفية بيضاء مع ظل */
    .glass-card {
        background: #ffffff;
        border-radius: 25px;
        padding: 20px;
    ...
    /* شريط التوصيات المتحرك - خلفية داكنة للتباين */
    .marquee {
...
LANG = {
    ...
    "en": {
        ...
        "weekend_msg": "Friday off", "inventory_header": "Inventory", "current_stock": "Current Stock",
        "receipt": "Receive", "material": "Material", "quantity": "Quantity", "invoice": "Invoice",
        ...
        "no_maintenance": "No maintenance records", "add_new_item": "Add Item", "item_id": "ID",
    ...
    "bn": {
        ...
        "receipt": "গ্রহণ", "material": "উপাদান", "quantity": "পরিমাণ", "invoice": "ইনভয়েস",
        "receipt_date": "তারিখ", "register_receipt": "নিবন্ধন", "low_stock_alert": "স্বল্প স্টক",
        "all_good": "সব ঠিক", "edit_stock": "স্টক সম্পাদনা", "new_stock": "নতুন স্টক",
        "update": "আপডেট", "stock_updated": "আপডেট হয়েছে", "export_btn": "এক্সপোর্ট",
        ...
        "no_maintenance": "রক্ষণাবেক্ষণ রেকর্ড নেই", "add_new_item": "আইটেম যোগ করুন",
        ...
        "auto_reorder": "স্বয়ংক্রিয় অর্ডার", "stock_prediction": "স্টক পূর্বাভাস",
...
def create_machine_file(filepath):
    if "Compressor" in filepath:
        sample = pd.DataFrame({
            "Category": ["فحوصات يومية", "فحص التشغيل", "تنظيف وتشحيم", "تنظيف وتشحيم", "فحص ميكانيكي", 
                        "فحص ميكانيكي", "أنظمة مساعدة", "أنظمة مساعدة", "كهرباء", "صيانة متقدمة", 
                        "صيانة متقدمة", "صيانة متقدمة"],
            "No": [1, 1, 1, 2, 1, 2, 1, 2, 1, 1, 2, 3],
            "Name": ["تعبئة سجل الأداء الأسبوعي", "فحص زر التوقف الطارئ", "فحص مستوى الزيت", 
                    "استبدال أو تنظيف فلتر الزيت", "فحص شد ومحاذاة السيور", "تنظيف فلتر هواء السحب",
                    "فحص عمل تصريف Bekomats", "تنظيف مبرد المجفف", "فلتر كابينة الكهرباء",
                    "عزم ربط أغطية الصمامات", "تشحيم رولمان بلي المحرك", "تنظيف المحرك الرئيسي"],
            "Photo": ["images/schedule.jpg", "images/estop.jpg", "images/oil.jpg", "images/oil_filter.jpg",
                     "images/belt.jpg", "images/air_filter.jpg", "images/bekomat.jpg", "images/cooler.jpg",
                     "images/cabinet_filter.jpg", "images/valve_cover.jpg", "images/grease.jpg", "images/motor_fan.jpg"],
            "Tools": ["قلم", "يدوي", "مقياس زيت", "مفتاح فلاتر", "مقياس شد", "هواء مضغوط", "فحص بصري", 
                     "فرشاة ناعمة", "مكنسة/هواء", "مفتاح عزم", "محقن شحم", "هواء مضغوط"],
            "Procedure": ["تسجيل البيانات", "اختبار يدوي", "فحص العلامة الصحيحة", "تنظيف أو استبدال",
                         "فحص الشد والمحاذاة", "تنظيف من الأتربة", "فحص عدم الانسداد", "تنظيف الزعانف",
                         "تنظيف للتهوية", "حسب المواصفات", "شحم كافٍ", "خالٍ من العوالق"],
            "Freq": ["Daily", "Daily", "Daily", "Weekly", "Weekly", "Weekly", "Weekly", "Weekly", "Weekly", "1000h", "Yearly", "Yearly"]
        })
...
def get_scheduled_tasks(df_tasks):
    ...
    if 'Freq' in df_tasks.columns:
        ...
        return df_tasks[df_tasks['Frequency'].isin(allowed)]
    return pd.DataFrame()
...
def get_marquee_recommendations(df_raw, df_main, df_fg):
    """توليد التوصيات لشريط البيانات المتحرك"""
    recommendations = []
    ...
    for rec in reorder[:3]:
        if rec["urgency"] == "high":
            recommendations.append(f"🔴 عاجل: أعد طلب {rec['material']} - الرصيد {rec['current']:,} (الحد الأدنى {rec['min_stock']:,})")
        else:
            recommendations.append(f"🟡 تنبيه: {rec['material']} يحتاج إعادة طلب - الكمية المقترحة {rec['suggested_qty']:,}")
    
    # توقع النفاذ
    stock_pred = get_stock_prediction(df_raw, df_main)
    for pred in stock_pred[:3]:
        if pred["status"] == "critical":
            recommendations.append(f"⚠️ خطر: {pred['material']} سينفذ خلال {pred['days_left']} يوم")
        elif pred["status"] == "warning":
            recommendations.append(f"📦 تنبيه: {pred['material']} سينفذ خلال {pred['days_left']} يوم")
    
    # حالة المنتج التام
    if not df_fg.empty and "Balance" in df_fg.columns:
        fg_balance = df_fg["Balance"].sum()
        if fg_balance <= 0:
            recommendations.append(f"🏭 تنبيه: مخزن المنتج التام فارغ - يرجى زيادة الإنتاج")
        elif fg_balance < 10000:
            recommendations.append(f"📦 مخزون المنتج التام منخفض: {fg_balance:,.0f} وحدة")
    
    if not recommendations:
        recommendations.append("✅ جميع المؤشرات جيدة - استمر بهذا المستوى! ✅")
    
    return recommendations

def show_marquee(df_raw, df_main, df_fg, t):
    """عرض شريط البيانات المتحرك"""
    recommendations = get_marquee_recommendations(df_raw, df_main, df_fg)
...
# ============================================================================
# 12. لوحة القيادة المبهرة (ألوان فاتحة)
# ============================================================================
def show_dashboard(df_main, df_raw, df_fg, t):
    ...
    # شريط التوصيات المتحرك
    show_marquee(df_raw, df_main, df_fg, t)
    ...
    with col1:
        ...
        <div class="glass-card">
            <div style="font-size: 1.2rem; opacity: 0.8; color: #1a1a2e;">🏭 {t['total_production']}</div>
            <div style="font-size: 3rem; font-weight: bold; color: #667eea;">{total_prod:,}</div>
            <div style="font-size: 0.8rem; color: #666;">إجمالي الإنتاج منذ البداية</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div class="glass-card">
            <div style="font-size: 1.2rem; opacity: 0.8; color: #1a1a2e;">📅 {t['monthly_production']}</div>
            <div style="font-size: 3rem; font-weight: bold; color: #f59e0b;">{monthly_prod:,}</div>
            <div style="font-size: 0.8rem; color: #666;">{datetime.now().strftime('%B %Y')}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.markdown(f"""
        <div class="glass-card">
            <div style="font-size: 1.2rem; opacity: 0.8; color: #1a1a2e;">🏭 {t['fg_balance']}</div>
            <div style="font-size: 3rem; font-weight: bold; color: #10b981;">{fg_balance:,}</div>
            <div style="font-size: 0.8rem; color: #666;">وحدة</div>
        </div>
        """, unsafe_allow_html=True)
            ...
            <div style="position: relative; width: 180px; height: 180px; margin: auto;">
                <svg width="180" height="180" viewBox="0 0 180 180">
                    ...
                    <circle cx="90" cy="90" r="78" fill="none" stroke="{color1}" stroke-width="14" 
                            stroke-dasharray="{2 * math.pi * 78 * line1_efficiency / 100} {2 * math.pi * 78}"
                    ...
                    <text x="90" y="118" text-anchor="middle" font-size="14" fill="#666">السرعة</text>
                </svg>
            </div>
            <small style="color: #666;">عدد السجلات: {line1_count}</small>
        </div>
        """, unsafe_allow_html=True)
            ...
            <div style="position: relative; width: 180px; height: 180px; margin: auto;">
                <svg width="180" height="180" viewBox="0 0 180 180">
                    ...
                    <circle cx="90" cy="90" r="78" fill="none" stroke="{color2}" stroke-width="14" 
                            stroke-dasharray="{2 * math.pi * 78 * line2_efficiency / 100} {2 * math.pi * 78}"
                            stroke-dashoffset="{2 * math.pi * 78 * 0.25}" transform="rotate(-90 90 90)"/>
                    <text x="90" y="95" text-anchor="middle" font-size="28" font-weight="bold" fill="{color2}">{line2_efficiency}%</text>
                    <text x="90" y="118" text-anchor="middle" font-size="14" fill="#666">السرعة</text>
                </svg>
            </div>
            <small style="color: #666;">عدد السجلات: {line2_count}</small>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.subheader("📦 " + t["raw_balance"] + " (بالتفصيل)")
        if not df_raw.empty:
            ...
            fig_raw.update_layout(height=450, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    ...
    with col2:
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.subheader("🏭 " + t["fg_balance"] + " (بالتفصيل)")
        if not df_fg.empty:
            ...
            fig_fg.update_layout(height=450, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
...
def main():
                ...
                if target <= 0:
                    ...
                    if not raw_ok:
                        ...
                        if update_raw_materials(new_raw):
                            ...
                            if conn:
                                save_to_sheet(conn, pd.concat([df_main, new_row], ignore_index=True) if not df_main.empty else new_row)
                            ...
                            if len(idx) > 0:
                                ...
                                df_fg.at[idx[0], "Last_Updated"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            ...
            if tasks.empty:
                ...
                with st.form("m_form"):
                    ...
                    for i, r in tasks.iterrows():
                        task_name = r.get('Name', r.get('Task', f'مهمة {i+1}')) if pd.notna(r.get('Name', r.get('Task', ''))) else f'مهمة {i+1}'
                        with st.expander(f"🔧 {task_name} ({r.get('Freq', 'N/A')})"):
                            col1, col2 = st.columns([3, 1])
                            with col1:
                                st.markdown(f"**{t['tools_label']}** {r.get('Tools', 'N/A')}")
                                st.info(f"**{t['proc_label']}**\n{r.get('Proc', r.get('Procedure', 'N/A'))}")
            ...
            with tab2:
                ...
                if not maint_logs.empty and 'Date' in maint_logs.columns:
                    ...
                    if not maint_logs_last10.empty:
                        st.dataframe(maint_logs_last10[['Date', 'Type', 'Machine', 'Technician', 'Task', 'Notes']], use_container_width=True)
    ...
    # ========== Finished Goods ==========
        ...
        if not df_fg.empty:
            ...
            display_df = display_df.rename(columns={
                'Name': 'المنتج', 'In': t['in'], 'Out': t['out'], 'Balance': t['balance'], 'Unit': 'الوحدة'
            })
            st.dataframe(display_df, use_container_width=True)
                ...
                if 'Type' in df_display.columns and 'Date' in df_display.columns:
                    def fmt(row):
                        if row['Type'] == 'Production':
                            return f"📦 إنتاج | {row['Date']} | {row.get('Product', 'N/A')} | {row.get('Output_Units', 0)} وحدة"
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v15.0<br>{t['designer']}</small></center>", unsafe_allow_html=True)
