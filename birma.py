            ...
            if not success:
                ...
                if update_inventory(df_inv_updated):
                    ...
                    new_row = pd.DataFrame([{
                       ...
                       "Staff": name, 
                       "Product": product, 
                       "Output_Units": int(target),  # تأكد من أنها رقم
                       "Waste_Bottles": int(preforms - total_b) if preforms > 0 else 0,  # رقم
                       "Waste_Raw": int(raw_val - target) if raw_val > 0 else 0,  # رقم
                       "Efficiency_%": float(eff),  # رقم عشري
                       "Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # نص فقط
                    }])
