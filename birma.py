...
def load_inventory():
    if os.path.exists("raw.xlsx"):
        ...
        return pd.DataFrame(columns=["Material_ID", "Material_Name_AR", "Current_Stock", "Min_Stock", "Unit", "Last_Updated"])
...
def consume_materials(product, quantity, df_inv):
    """صرف المواد من المخزون (BOM معرف كمتغير عام)"""
...
def main():
    ...
    # ========== Inventory ==========
        ...
        if not df_inv.empty:
            ...
            with tab1:
                st.dataframe(df_inv[["Material_Name_AR", "Current_Stock", "Min_Stock", "Unit", "Last_Updated"]], use_container_width=True)
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v10.3<br>{t['designer']}</small></center>", unsafe_allow_html=True)
