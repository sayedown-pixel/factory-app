...
# --- 2. نظام حفظ الحالة عبر URL ---
def get_auth_token(username, password):
    """إنشاء رمز مصادقة فريد"""
    secret = "BIRMA_SECRET_KEY_2024"
    token_string = f"{username}:{password}:{secret}"
    return hashlib.sha256(token_string.encode()).hexdigest()[:32]

def verify_auth_token(token, username, password):
    """التحقق من صحة رمز المصادقة"""
    expected = get_auth_token(username, password)
    return token == expected

def save_auth_to_url(username, password, remember=True):
    """حفظ بيانات المصادقة في URL"""
    if remember:
        token = get_auth_token(username, password)
        # إضافة المعاملات إلى URL
        st.query_params["auth_token"] = token
        st.query_params["auth_user"] = username
        st.query_params["auth_remember"] = "true"
        return True
    return False

def load_auth_from_url():
    """تحميل بيانات المصادقة من URL"""
    if "auth_token" in st.query_params and "auth_user" in st.query_params:
        ...
        remember = st.query_params.get("auth_remember", "") == "true"
        
        # التحقق من وجود المستخدم
        if username in USERS:
            # التحقق من صحة الرمز
            if verify_auth_token(token, username, USERS[username]["password"]):
                return username, USERS[username]["password"], remember
    return None, None, False

def clear_auth_from_url():
    """مسح بيانات المصادقة من URL"""
    keys_to_remove = ["auth_token", "auth_user", "auth_remember"]
...
LANG = {
    "ar": {
        ...
        "remember_me": "تذكرني (تسجيل تلقائي لاحقاً)",
        "clear_saved": "مسح البيانات المحفوظة",
        "info_title": "ℹ️ معلومات عن حفظ البيانات",
        "info_text": """
        📱 **كيفية حفظ بيانات الدخول على التليفون:**
        
        1️⃣ قم بتسجيل الدخول مع تفعيل خيار "تذكرني"
        2️⃣ سيتم حفظ بياناتك في رابط التطبيق
        3️⃣ في المرة القادمة، افتح نفس الرابط وسجل دخول تلقائياً
        4️⃣ يمكنك حفظ الرابط في المفضلة لسهولة الوصول
        
        ⚠️ **ملاحظة:** لا تشارك الرابط مع الآخرين لأنه يحتوي على رمز الدخول الخاص بك.
        """
    },
    "en": {
        ...
        "item_unit": "Unit",
        "min_stock": "Min Stock",
        "remember_me": "Remember me (auto-login next time)",
        "clear_saved": "Clear saved data",
        "info_title": "ℹ️ About data saving",
        "info_text": """
        📱 **How to save login data on phone:**
        
        1️⃣ Login with "Remember me" option enabled
        2️⃣ Your login token will be saved in the URL
        3️⃣ Next time, open the same URL to auto-login
        4️⃣ You can bookmark the URL for easy access
        
        ⚠️ **Note:** Do not share the URL as it contains your login token.
        """
    },
    "bn": {
        ...
        "info_title": "ℹ️ তথ্য",
        "info_text": """
        📱 **কিভাবে লগইন ডেটা সংরক্ষণ করবেন:**
        
        1️⃣ "আমাকে মনে রাখুন" অপশন সহ লগইন করুন
        2️⃣ আপনার লগইন টোকেন URL-এ সংরক্ষিত হবে
        3️⃣ পরবর্তী সময়ে, একই URL খুললে স্বয়ংক্রিয় লগইন হবে
        4️⃣ URL বুকমার্ক করে রাখতে পারেন
        
        ⚠️ **দ্রষ্টব্য:** URL অন্যদের সাথে শেয়ার করবেন না।
        """
    }
}
...
def calculate_spacers_needed(product, quantity_produced):
    ...
    num_pallets = math.ceil(quantity_produced / config["units_per_pallet"])
    return num_pallets * config["spacers_per_pallet"]

def calculate_adhesive_needed(product, quantity_produced):
    if product not in ADHESIVE_CONSUMPTION:
        return 0
    total_kg = (ADHESIVE_CONSUMPTION[product] * quantity_produced) / 1000
...
def calculate_hotmelt_needed(product, quantity_produced):
    if product not in HOTMELT_CONSUMPTION:
        return 0
    total_kg = (HOTMELT_CONSUMPTION[product] * quantity_produced) / 1000
    return math.ceil(total_kg * 100) / 100
...
def login_screen(t):
    # محاولة تحميل البيانات المحفوظة من URL
    saved_user, saved_pass, has_creds = load_auth_from_url()
    
    # إذا كانت هناك بيانات محفوظة وصالحة، سجل دخول تلقائي
            ...
            with col_btn1:
                if st.form_submit_button(t["login_btn"], use_container_width=True):
                    if username in USERS and USERS[username]["password"] == password:
                        ...
                        if remember:
                            save_auth_to_url(username, password, True)
                        else:
                            clear_auth_from_url()
                        st.rerun()
                    else:
    ...
    with st.expander(t["info_title"]):
        ...
        # عرض الرابط الحالي للمستخدم
        current_url = st.query_params.to_dict()
        if current_url:
            st.info(f"🔗 الرابط الحالي يحتوي على رمز الدخول: `{dict(current_url)}`")
...
def main():
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v10.7<br>{t['designer']}</small></center>", unsafe_allow_html=True)
