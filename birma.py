...
# BOM الأساسية مع إضافة فواصل الشرنك (سيتم حسابها بشكل منفصل)
BOM = {
...
def consume_materials(product, quantity, df_inv):
    """صرف المواد من المخزون (مع دعم فواصل الشرنك)"""
    ...
    # حساب فواصل الشرنك بشكل خاص (للمنتجات من نوع شرنك فقط)
...
def main():
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v10.4<br>{t['designer']}</small></center>", unsafe_allow_html=True)
