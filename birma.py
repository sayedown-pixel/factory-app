...
if app_mode == LANG[ln]["mode_prod"]:
    ...
    # التحقق من الباسورد (كلمة السر للإنتاج: birma2026)
    if user_password == "birma2026":
        ...
        st.error(LANG[ln]["pass_error"])

    # عرض السجل متاح دائماً للقراءة فقط
    st.divider()
    ...
    try:
        ...
        if not history_df.empty:
            st.dataframe(history_df[history_df['Type'] == 'Production'].tail(10)[::-1], use_container_width=True)
...
# --- 6. قسم الصيانة ---
    ...
    # التحقق من الباسورد (كلمة السر للصيانة: maint2026)
        ...
        if os.path.exists(file_path):
            ...
            with st.form("maint_form"):
                ...
                for i, row in daily.iterrows():
                    ...
                    with c_note: note = st.text_input("Note", key=f"n_{i}")
                    if done:
