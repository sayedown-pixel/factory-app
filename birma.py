...
# --- 2. نظام حفظ البيانات على التليفون ---
    ...
    if remember:
        ...
        st.markdown(f"""
            <script>
                ...
                sessionStorage.setItem('birma_logged', 'true');
...
def clear_saved_credentials():
    ...
    st.markdown("""
        <script>
            ...
            sessionStorage.removeItem('birma_logged');
...
def is_remembered_session():
    """التحقق من وجود جلسة محفوظة"""
    return 'session' in st.query_params
...
LANG = {
    "ar": {
        ...
        2️⃣ سيتم حفظ بياناتك في متصفح التليفون
        3️⃣ في المرة القادمة، سيتم تسجيل الدخول تلقائياً
        4️⃣ البيانات محفوظة محلياً فقط ولا تخرج من جهازك
        
        ⚠️ **ملاحظة:** إذا قمت بمسح بيانات المتصفح، ستحتاج لإعادة تسجيل الدخول.
        """
    },
    "en": {
        ...
        2️⃣ Your data will be saved in your phone browser
        3️⃣ Next time, you'll be logged in automatically
        4️⃣ Data is stored locally only and never leaves your device
        
        ⚠️ **Note:** If you clear browser data, you'll need to login again.
        """
    },
    "bn": {
        ...
        2️⃣ আপনার ডেটা আপনার ফোন ব্রাউজারে সংরক্ষিত হবে
        3️⃣ পরবর্তী সময়ে, আপনি স্বয়ংক্রিয়ভাবে লগইন হবেন
        4️⃣ ডেটা শুধুমাত্র স্থানীয়ভাবে সংরক্ষিত হয়
        
        ⚠️ **দ্রষ্টব্য:** ব্রাউজার ডেটা মুছে ফেললে, আবার লগইন করতে হবে।
        """
    }
}
...
def login_screen(t):
    ...
    saved_user, saved_pass, has_creds = load_saved_credentials()
    has_session = is_remembered_session()
    
    # إذا كانت هناك جلسة سابقة، سجل دخول تلقائي
    if has_session and saved_user and saved_pass:
            ...
            with col_btn1:
                if st.form_submit_button(t["login_btn"], use_container_width=True):
                    if username in USERS and USERS[username]["password"] == password:
                        ...
                        if remember:
                            save_credentials(username, password, True)
                        st.rerun()
                    else:
...
def main():
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v10.6<br>{t['designer']}</small></center>", unsafe_allow_html=True)
