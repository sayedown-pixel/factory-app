...
# --- 4. الربط والخدمات (تيليجرام وذكاء اصطناعي) ---
try:
...
# --- 5. الإعدادات الفنية ---
CONFIG = {
    "الخط الأول(smi line)": {"الأصناف": ["200 ml Carton", "200 ml Shrink", "600 ml Carton", "1.5 L Shrink"],
...
# --- 6. واجهة المستخدم ---
selected_menu = st.sidebar.selectbox("القائمة", LANG[ln]["menu"])
...
if selected_menu == LANG[ln]["menu"][0]:
    ...
    with st.form("prod_form"):
        ...
        if st.form_submit_button(LANG[ln]["save_btn"]):
            ...
            waste_p = preforms - total_b
            waste_r = raw_val - target
            
            new_row = pd.DataFrame([{"Type": "Production", "Line": selected_line, "Date": str(p_date), "Staff": name, 
                                     "Product": product, "Output_Units": target, "Waste_Bottles": waste_p, 
                                     "Waste_Raw": waste_r, "Efficiency_%": eff, "Timestamp": datetime.now().strftime("%H:%M")}])
            ...
            # إرسال تيليجرام
            t_msg = f"🚀 *{LANG[ln]['menu'][0]}*\nLine: {selected_line}\nProduct: {product}\nOutput: {target}\nEfficiency: {eff}%\nPreform Waste: {waste_p}\nSup: {name}"
            send_telegram(t_msg)
...
# --- ب. الصيانة ---
    ...
    if m_type == LANG[ln]["maint_types"][0]: # دورية
            ...
            with st.form("m_form"):
                ...
                for i, r in daily.iterrows():
                    ...
                    with c_i:
                        ...
                        note = st.text_input(LANG[ln]["note_label"], key=f"n{i}") # استعادة الملاحظات
                    with c_p:
                ...
                if st.form_submit_button(LANG[ln]["save_btn"]):
                    ...
                    send_telegram(f"🔧 *Daily Maintenance*\nMachine: {machine}\nTech: {tech}\nStatus: Completed")
                    st.success(LANG[ln]["success_msg"]); st.rerun()
        ...
        with st.form("break_form"):
            ...
            m_note = st.text_area(LANG[ln]["note_label"]) # استعادة الملاحظات
            if st.form_submit_button(LANG[ln]["save_btn"]):
                new_b = pd.DataFrame([{"Type": "Maint_Breakdown", "Line": selected_line, "Date": str(datetime.now().date()), "Machine": machine, "Staff": t_name, "Notes": f"Time: {t1}-{t2} | Issue: {issue} | Note: {m_note}"}])
                ...
                send_telegram(f"⚠️ *Breakdown Report*\nMachine: {machine}\nDowntime: {t1} to {t2}\nIssue: {issue}\nTech: {t_name}")
                st.success(LANG[ln]["success_msg"]); st.rerun()
...
# --- ج. السجلات والتقارير ---
    ...
    if not df_main.empty:
        ...
        if not prod_data.empty:
            g1, g2 = st.columns(2)
            with g1:
                fig_g = go.Figure(go.Indicator(mode="gauge+number", value=prod_data['Efficiency_%'].mean(), 
                                             title={'text': LANG[ln]["eff_title"]}, gauge={'bar':{'color':"green"}}))
                st.plotly_chart(fig_g, use_container_width=True)
            with g2:
                fig_b = px.bar(prod_data, x='Date', y='Waste_Bottles', color='Product', title=LANG[ln]["waste_title"])
                st.plotly_chart(fig_b, use_container_width=True)
            
            st.plotly_chart(px.line(prod_data, x='Date', y='Efficiency_%', color='Line', markers=True, title="Efficiency Evolution"), use_container_width=True)

        st.divider()
        tab1, tab2 = st.tabs([LANG[ln]["history_p"], LANG[ln]["history_m"]])
        with tab1: st.dataframe(df_main[df_main['Type'] == 'Production'].tail(20)[::-1], use_container_width=True)
        with tab2: st.dataframe(df_main[df_main['Type'].str.contains('Maint', na=False)].tail(20)[::-1], use_container_width=True)

# --- التذييل (Footer) ---
st.markdown(f"<br><hr><center><p style='color: gray;'>BIRMA v5.5 | <b>Designed by: {LANG[ln]['designer']}</b></p></center>", unsafe_allow_html=True)