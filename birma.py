...
# ============================================================================
# 1. إعدادات الصفحة (بدون خلفية ملونة)
# ============================================================================
...
<style>
    ...
    .marquee { background: #1a1a2e; border-radius: 30px; padding: 10px 20px; margin: 20px 0; overflow: hidden; white-space: nowrap; color: white; }
...
LANG = {
    ...
    "bn": {
        ...
        "history_m": "রক্ষণাবেক্ষণ লগ", "history_delivery": "ডেলিভারি লগ",
        "admin_title": "অ্যাডমিন - মুছুন", "delete_btn": "মুছুন", "del_success": "মুছে ফেলা হয়েছে",
        "tools_label": "সরঞ্জাম:", "proc_label": "পদ্ধতি:", "weekend_msg": "শুক্রবার ছুটি",
        "inventory_header": "ইনভেন্টরি", "current_stock": "বর্তমান স্টক", "receipt": "গ্রহণ",
        "material": "উপাদান", "quantity": "পরিমাণ", "invoice": "ইনভয়েস", "receipt_date": "তারিখ",
        ...
        "total_production": "মোট উৎপাদন", "monthly_production": "মাসিক উৎপাদন",
        "line1_efficiency": "লাইন ১ দক্ষতা", "line2_efficiency": "লাইন ২ দক্ষতা",
        "smart_recommendations": "স্মার্ট সুপারিশ", "users_title": "ব্যবহারকারী",
        "settings_title": "সেটিংস", "backup_data": "ব্যাকআপ", "clear_cache": "ক্যাশ সাফ",
        "machine_select": "মেশিন নির্বাচন", "task_name": "কাজ", "done": "সম্পন্ন",
        "no_data": "তথ্য নেই", "no_production": "উৎপাদন রেকর্ড নেই",
        "no_maintenance": "রক্ষণাবেক্ষণ রেকর্ড নেই", "no_delivery": "ডেলিভারি রেকর্ড নেই",
        "add_new_item": "নতুন আইটেম", "item_id": "আইডি", "item_name": "নাম",
        "item_unit": "ইউনিট", "min_stock": "ন্যূনতম স্টক", "info_title": "তথ্য",
        "info_text": "স্থানীয়ভাবে ডেটা সংরক্ষিত", "shipping": "প্রেরণ", "customer": "গ্রাহক",
        ...
        "clear_saved": "সংরক্ষিত ডেটা মুছুন", "auto_reorder": "স্বয়ংক্রিয় অর্ডার",
        "stock_prediction": "স্টক শেষ", "raw_balance": "কাঁচামাল", "fg_balance": "সমাপ্ত পণ্য",
        "delivery": "ডেলিভারি", "product": "পণ্য", "quantity_to_deliver": "ডেলিভারির পরিমাণ",
        "manual_adjust": "ম্যানুয়াল", "waste_bottles": "বর্জ্য বোতল"
    }
}
...
def create_default_raw_materials():
    default_data = {
        ...
        "Current_Stock": [500000, 300000, 100000, 50000, 2410104, 500000, 300000, 50000],
...
# ============================================================================
# 11. التوصيات الذكية (مع تصحيح حساب الغطاء)
# ============================================================================
...
def get_stock_prediction_calculated(df_raw, df_main, selected_line):
    """حساب توقع نفاذ المخزون - مع استهلاك ثابت للغطاء 900,000/يوم"""
    predictions = []
    
    if df_main is None or df_main.empty or df_raw.empty:
        return predictions
    ...
    # ========== حساب استهلاك الغطاء أولاً (مرة واحدة فقط) ==========
    ...
    for product in products:
        ...
        if product in BOM:
            for material, qty in BOM[product].items():
                if material == "غطاء":
                    continue  # تخطي الغطاء لأنه تم حسابه بالفعل
                elif qty < 1:
...
def show_dashboard(df_main, df_raw, df_fg, t, selected_line):
    ...
    with col1:
        st.metric(t["line1_efficiency"], f"{line1_efficiency}%")
    with col2:
        st.metric(t["line2_efficiency"], f"{line2_efficiency}%")
...
def main():
                ...
                if target <= 0:
                    ...
                    if not raw_ok:
                        ...
                        if update_raw_materials(new_raw):
                            ...
                            fg_name = get_finished_goods_name(product)
    ...
    # ========== Records (آخر 10 أيام - مع سجل التحميل) ==========
    ...
    st.sidebar.divider()
    st.sidebar.markdown(f"<center><small>BIRMA v21.0<br>{t['designer']}</small></center>", unsafe_allow_html=True)
