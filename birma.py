...
def send_telegram(msg):
    try:
        token = st.secrets["telegram"]["bot_token"]; chat_id = st.secrets["telegram"]["chat_id"]
...
# --- 5. منطق فلترة مواعيد الصيانة (جديد) ---
def get_scheduled_tasks(df_tasks):
    today = datetime.now()
    day_name = today.strftime('%A') # الحصول على اسم اليوم بالإنجليزية
    ...
    # تحديد الترددات المطلوبة بناءً على اليوم
    allowed_freqs = ['Daily'] # اليومي يظهر كل الأيام ماعدا الجمعة
...
# ب. الصيانة (المحدث بالمواعيد)
    ...
    if m_type == LANG[ln]["maint_types"][0]: # دورية مجدولة
    ...
    if pw == "admin123":
        if not df_main.empty:
            ...
            if st.button(LANG[ln]["delete_btn"]):
                df_updated = df_main.drop(row_to_del); conn.update(spreadsheet=st.secrets["connections"]["gsheets_testing"]["spreadsheet"], data=df_updated)
                st.success(LANG[ln]["del_success"]); st.rerun()

st.markdown(f"<br><hr><center><p style='color: gray;'>BIRMA v6.0 | <b>Designed by: {LANG[ln]['designer']}</b></p></center>", unsafe_allow_html=True)