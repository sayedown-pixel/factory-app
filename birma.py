...
def get_scheduled_tasks(df_tasks):
    ...
    if 'Freq' not in df_tasks.columns and 'Frequency' not in df_tasks.columns:
        return pd.DataFrame()
    
    freq_col = 'Freq' if 'Freq' in df_tasks.columns else 'Frequency'
    
    # الجمعة: لا مهام
    if day_name == 'Friday':
        return pd.DataFrame()
    
    # المهام اليومية تظهر كل الأيام ما عدا الجمعة
    allowed_freqs = ['Daily']
    
    # المهام الأسبوعية تظهر يوم السبت فقط
    if day_name == 'Saturday':
        allowed_freqs.append('Weekly')
    
    # المهام الشهرية والسنوية تظهر أول الشهر
    ...
    result = df_tasks[df_tasks[freq_col].isin(allowed_freqs)].copy()
